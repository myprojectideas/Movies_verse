...
export default function Movie() {
    ...
    useEffect(() => {
        const fetchData = () => {
            ...
            fetch(`https://api.themoviedb.org/3/tv/${params.get('id')}?language=en-US`, options)
    ...
    return (
        <div className='flex w-full h-full px-[5%] gap-4 flex-wrap overflow-auto'>
            ...
            <div className='lg:flex-1 md:flex-1 sm:w-full px-4 flex flex-col gap-3'>
                ...
                <div className='text-start ml-4'>Number Of Episodes: {data.numberOfEpisodes}</div>
                <div className='text-start ml-4'>Number Of Seasons: {data.numberOfSeasons}</div>
                <div className='text-start ml-4'>Status: {data.status}</div>
